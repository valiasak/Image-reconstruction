...
dopplercenter = 250;
pi = 3.141592654;
header = sprintf('%d %d %d \n', cols, rows, cols);
prompt = 'please type s for slice, or c for crush image reconstruction \n';
image_type=input(prompt,'s');
...
limits = -slicethickness*mesh:slicethickness*mesh;
for j = 1:rows
    j
    for i = 1:cols
        ...
        for k = limits   % full crush, no slice
...
if image_type == 'c'
    ...
    colorbar
    saveas(gcf,str1)
    str2txt = sprintf('%scrush.txt', filename);
    fid = fopen(str2txt, 'w');
    fprintf(fid, header);
    fclose(fid);
    dlmwrite(str2txt,olo,'-append','delimiter',' ');
    dlmwrite(str2txt,oloc,' ');
    ...
    %imwrite(oloc,str2)
    saveas(gcf,str2)
    str2txt = sprintf('%sslice.txt', filename);
    fid = fopen(str2txt, 'w');
    fprintf(fid, header);
    fclose(fid);
    dlmwrite(str2txt,olo,'-append','delimiter',' ');
